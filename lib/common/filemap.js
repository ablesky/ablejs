...
exports.addAmdDepends = function(identifier, deps) {
    if (Array.isArray(deps)) {
        deps.map(function() {
            var suffix = /\.js$/;
            return suffix.test(this) ? this : this + '.js';
        });
...
exports.processDepends = function(wdata, wdepends) {
    wdepends.map(function(element) {
        // TODO sort element, in case of replace error.
        return element;
    }).forEach(function(element) {
        if (filemap[element]) {
            wdata = wdata.replace(new RegExp(element, 'g'), filemap[element].fingerprint + path.extname(element));
        }
    });
