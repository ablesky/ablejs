/**
 * A grunt task that generates file fingerprint using CRC32(32 - bit Cyclic Redundancy Check).
...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('fingerprint', 'generates file fingerprint by crc32 checksum.', function() {
        ...
        this.files.forEach(function(element, i, array) {
            ...
            element.src.forEach(function(relpath, i, array) {
                ...
                if (!fs.existsSync(originPath)) {
                    return log.warn('Source file "' + originPath + '" not found.');
                } else {
                ...
                if (fileType === '') {
                    return log.warn('There\'s sth wrong with source file: "' + destPath + '".');
                } else {
                    hashmap[fileType][moduleName] = hash;

                    log.info('generating hash file: ' + newpath);
