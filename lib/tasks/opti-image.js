	...
	grunt.registerMultiTask('optiIMG', 'Optimize images', function() {
		...
		function optimizeImg(binPath, sourceFile, resultFile, options, callback) {
			execFile(binPath, options, function(err, stdout, stderr) {
				...
				var sourceSize = fs.statSync(sourceFile).size;
				var resultSize = fs.statSync(resultFile).size;
				var saved = sourceSize - resultSize;
resultSize
				log.writeln('source size: ' + (sourceSize / 1000 + 'kb').cyan +
					'. result size: ' + (resultSize / 1000 + 'kb').cyan + '.  saving: ' + (saved / 1000 + 'kb').cyan + ' . \n');
