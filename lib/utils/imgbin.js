...
exports.optimize = function(sourceFile, resultFile, callback) {
    ...
    if (fs.existsSync(resultFile)) {
        ...
        // for font file as image, such as woff,ttf,svg and eot
        if (binPath === 'fontFile') {
            file.copy(sourceFile, resultFile);
            log.info('just copy source(font) file: "' + sourceFile + '" to result path.\n');
            callback();
            return;
        }
        ...
        execFile(binPath, options, function(err, stdout, stderr) {
            ...
            if (err) {
                ...
                log.info('just copy source file: "' + sourceFile + '" to result path.\n');
            } else {
