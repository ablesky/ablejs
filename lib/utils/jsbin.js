...
/**
 * @example getBaseUrl(moduleName, modulePath)
 */
function getBaseUrl(mod, p) {
    var modArr = mod.split('/');
    var pathBuildArr = [p];

    modArr.forEach(function (elem, i) {
        pathBuildArr.push('..');
    });

    return path.join.apply(null, pathBuildArr);
}
...
exports.optimize = function(sourcePath, resultPath, identifier, options, callback) {
    ...
    // deal with the relative path by path pkg, either linux or windows can work well
    var baseUrl = getBaseUrl(identifier, sourcePath);
